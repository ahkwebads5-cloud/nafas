...
module.exports = async (req, res) => {
  ...
  var CURSYM = { EGP:'ج.م', SAR:'ر.س', AED:'د.إ', KWD:'د.ك', QAR:'ر.ق', BHD:'د.ب', OMR:'ر.ع', JOD:'د.أ', LBP:'ل.ل', IQD:'د.ع', MAD:'د.م', DZD:'د.ج', TND:'د.ت', LYD:'د.ل', SDG:'ج.س', YER:'ر.ي', USD:'$', EUR:'€' };
  var c = CURSYM[d.currency] || 'ج.م';
  const ctx = 'بيانات المشروع:\n' +
    '- الاسم: ' + (d.name || 'غير محدد') + ' | القطاع: ' + (d.sector || '') + ' | المرحلة: ' + (d.stage || '') + ' | البلد: ' + (d.country || '') + '\n' +
    '- الكاش المتاح: ' + num(d.cash) + ' ' + c + ' | المصاريف الشهرية: ' + num(d.expenses) + ' ' + c + ' | الإيراد الشهري: ' + num(d.revenue) + ' ' + c + '\n' +
    '- نمو الإيراد الشهري: ' + (d.growth || 0) + '% | النفس الحالي: ' + (d.runway >= 999 ? 'مؤمّن' : (d.runway + ' شهر')) + ' | المبلغ الناقص لهدف ' + (d.targetMonths || 18) + ' شهر: ' + num(d.needed) + ' ' + c + '\n';
